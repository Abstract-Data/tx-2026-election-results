"""Data migration script to load parquet data into database."""
import asyncio
...
async def migrate_voters(
    session: AsyncSession, voterfile_path: Path, observers: List
) -> None:
    """
    Migrate voter data from parquet to database.

    Args:
        session: Database session
        voterfile_path: Path to processed voterfile parquet
        observers: List of migration observers
    """
    print("Loading voterfile...")
    df = pl.read_parquet(voterfile_path)

    total = len(df)
    print(f"Processing {total} voters...")

    # Create likelihood scorer
    ...
    for batch_start in range(0, total, batch_size):
        ...
        for row in batch_df.iter_rows(named=True):
            try:
                ...
                # Prepare voter data for scoring
                voter_data = {
                    "pri24": row.get("PRI24", ""),
                    "pri22": row.get("PRI22", ""),
                    "age": row.get("age"),
...
async def migrate_early_voting(
    session: AsyncSession, early_voting_path: Path, observers: List
) -> None:
    """
    Migrate early voting data from parquet to database.

    Args:
        session: Database session
        early_voting_path: Path to processed early voting parquet
        observers: List of migration observers
    """
    print("Loading early voting data...")
    ...
    # Update voter records with actual_voted_early flag
...
async def migrate_turnout_metrics(
    ...
    Args:
        session: Database session
        turnout_dir: Directory containing turnout CSV files
        observers: List of migration observers
    """
    print("Loading turnout metrics...")

    # Map CSV files to district types
    turnout_files = {
...
async def main():
    ...
    async with session_factory() as session:
        try:
            # Migrate voters
            await migrate_voters(session, Config.PROCESSED_VOTERFILE, observers)

            # Migrate early voting
            await migrate_early_voting(session, Config.PROCESSED_EARLY_VOTING, observers)

            # Migrate turnout metrics
